  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto w-full pb-32 animate-in fade-in duration-500">
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <button onClick={() => setShowLangModal(true)} className="w-full flex items-center justify-between p-5 bg-transparent hover:bg-slate-800/50 active:bg-slate-800/80 transition-colors border-b border-slate-800/80 group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-emerald-500/10 flex items-center justify-center text-emerald-500 group-hover:bg-emerald-500 group-hover:text-white transition-colors duration-300">
                <Globe size={20} />
              </div>
          ...
          <button onClick={() => { setStatus({ type: 'success', msg: '' }); setShowDataModal(true); }} className="w-full flex items-center justify-between p-5 bg-transparent hover:bg-slate-800/50 active:bg-slate-800/80 transition-colors border-b border-slate-800/80 group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-blue-500/10 flex items-center justify-center text-blue-500 group-hover:bg-blue-500 group-hover:text-white transition-colors duration-300">
                <FileJson size={20} />
              </div>
              <div className="flex flex-col text-left">
          ...
          {/* Family Management */}
          <button onClick={() => setShowFamilyModal(true)} className="w-full flex items-center justify-between p-5 bg-transparent hover:bg-slate-800/50 active:bg-slate-800/80 transition-colors group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-pink-500/10 flex items-center justify-center text-pink-500 group-hover:bg-pink-500 group-hover:text-white transition-colors duration-300">
                <Users size={20} />
              </div>
              <div className="flex flex-col text-left">
        ...
        >
          <button onClick={() => setShowDeleteChatModal(true)} className="w-full flex items-center justify-between p-5 bg-transparent hover:bg-red-500/5 active:bg-red-500/10 transition-colors group">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-red-500/10 flex items-center justify-center text-red-500 group-hover:bg-red-500 group-hover:text-white transition-colors duration-300">
      ...
      <AnimatePresence>
        {showDataModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="bg-slate-900/90 backdrop-blur-xl rounded-3xl w-full max-w-lg flex flex-col shadow-2xl shadow-black/80 border border-slate-800">
              <div className="p-5 border-b border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-500"><FileJson size={16} /></div>
                  <h2 className="text-lg font-bold text-white">{t('settings.data_management')}</h2>
                </div>
                <button onClick={() => setShowDataModal(false)} className="w-8 h-8 rounded-full bg-slate-800 text-slate-400 flex items-center justify-center hover:bg-slate-700 hover:text-white transition-all active:scale-90"><X size={18} /></button>
              </div>
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-slate-800/40 rounded-2xl p-5 flex flex-col items-center text-center border border-slate-700/50 hover:bg-slate-800/60 transition-colors group">
                    <div className="w-12 h-12 bg-slate-800 rounded-xl flex items-center justify-center text-slate-400 mb-4 group-hover:text-white transition-colors"><Download size={24} /></div>
                    ...
                    <button onClick={handleExport} disabled={loadingData} className="w-full px-4 py-3 bg-slate-700 hover:bg-slate-600 text-white rounded-xl font-bold transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 text-sm">
                  ...
                  <div className="bg-slate-800/40 rounded-2xl p-5 flex flex-col items-center text-center border border-slate-700/50 hover:border-red-500/30 transition-colors group">
                    <div className="w-12 h-12 bg-red-500/10 rounded-xl flex items-center justify-center text-red-500 mb-4 group-hover:bg-red-500/20 transition-colors"><Upload size={24} /></div>
                    ...
                    <label className={`w-full px-4 py-3 bg-gradient-to-br from-red-500 to-red-700 hover:from-red-400 hover:to-red-600 text-white rounded-xl font-bold transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer text-sm ${loadingData ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        {showLangModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="bg-slate-900/90 backdrop-blur-xl rounded-3xl w-full max-w-md flex flex-col shadow-2xl shadow-black/80 border border-slate-800">
              <div className="p-5 border-b border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-emerald-500/20 flex items-center justify-center text-emerald-500"><Globe size={16} /></div>
                  <h2 className="text-lg font-bold text-white">{t('settings.language')}</h2>
                </div>
                <button onClick={() => setShowLangModal(false)} className="w-8 h-8 rounded-full bg-slate-800 text-slate-400 flex items-center justify-center hover:bg-slate-700 hover:text-white transition-all active:scale-90"><X size={18} /></button>
              </div>
              <div className="p-6 space-y-3">
                <button onClick={() => { setLanguage('th'); setShowLangModal(false); }} className={`w-full flex items-center justify-between px-5 py-4 rounded-xl border font-bold transition-all ${language === 'th' ? 'bg-red-500/10 border-red-500 text-red-500' : 'bg-slate-800/50 border-slate-700 text-slate-300 hover:bg-slate-700 hover:text-white'}`}>
                  <span>ภาษาไทย (TH)</span>
                  {language === 'th' && <CheckCircle2 size={20} />}
                </button>
                <button onClick={() => { setLanguage('en'); setShowLangModal(false); }} className={`w-full flex items-center justify-between px-5 py-4 rounded-xl border font-bold transition-all ${language === 'en' ? 'bg-red-500/10 border-red-500 text-red-500' : 'bg-slate-800/50 border-slate-700 text-slate-300 hover:bg-slate-700 hover:text-white'}`}>
      ...
      <AnimatePresence>
        {showDeleteChatModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[60] flex items-center justify-center p-4" onClick={() => !deletingChat && setShowDeleteChatModal(false)}>
            <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="bg-slate-900/90 backdrop-blur-xl rounded-3xl w-full max-w-sm flex flex-col shadow-2xl shadow-black/80 border border-red-900/40" onClick={(e) => e.stopPropagation()}>
              <div className="p-6 flex flex-col items-center text-center">
                ...
                <p className="text-slate-400 text-sm font-medium leading-relaxed mb-8">
                  {language === 'th' ? 'ข้อความแชททั้งหมดจะถูกลบอย่างถาวร และไม่สามารถกู้คืนได้ คุณยืนยันที่จะลบหรือไม่?' : 'All chat messages will be permanently deleted and cannot be recovered. Are you sure?'}
                </p>
                <div className="flex gap-3 w-full">
                  <button onClick={() => setShowDeleteChatModal(false)} disabled={deletingChat} className="flex-1 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all active:scale-95 disabled:opacity-50">
                    {language === 'th' ? 'ยกเลิก' : 'Cancel'}
                  </button>
                  <button onClick={handleDeleteChat} disabled={deletingChat} className="flex-1 py-3 bg-gradient-to-br from-red-600 to-red-800 hover:from-red-500 hover:to-red-700 text-white font-bold rounded-xl transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2">
                    {deletingChat && <Loader2 size={16} className="animate-spin" />}
